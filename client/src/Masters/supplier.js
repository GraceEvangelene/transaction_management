...
const Supplier = () => {
  ...
  useEffect(() => {
    const fetchLatestSupplierCode = async () => {
      try {
        const response = await axios.get(`${api_base}/latestSupplierCode`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${api_base}/Masters/supplier`, formData, {
        headers: {
  ...
  <div class="form" onSubmit={handleSubmit}>
      <h1 class="title">Create New Supplier</h1>
      ...
      <div class='view'>
              <button id="view" onClick={() => navigateToSuppliers()}>View All</button>
          </div>
  </div>
</div>
  );
